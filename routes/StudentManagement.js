...
router.post("/generate-certificate/:id", async (req, res) => {
  try {
    ...
    // Register Times New Roman font (uncomment if you have the font file)
    ...
    stream.on("finish", () => {
      ...
      res.download(pdfPath, `${type}_${fullName}.pdf`, (err) => {
        ...
        // Clean up PDF file
        fs.unlink(pdfPath, (cleanupErr) => {
          if (cleanupErr) console.error("Error cleaning up PDF:", cleanupErr);
        });
      });
    });
